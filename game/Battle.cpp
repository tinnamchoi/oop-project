...
void Battle::move(int moveChoice) {
  ...
  if (computer->pokemon[computer->currentPokemon].health <= 0 && winState() == 0) {
...
void Battle::move() {
  ...
  if (person->pokemon[person->currentPokemon].health <= 0 && winState() == 0) {
    std::cout << "Your pokemon is dead!" << std::endl;
    move(4);
  }
}
